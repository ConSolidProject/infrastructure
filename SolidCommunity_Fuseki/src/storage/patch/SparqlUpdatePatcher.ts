...
export class SparqlUpdatePatcher extends RepresentationPatcher {
  ...
  private async patch({ identifier, patch, representation }: RepresentationPatcherInput): Promise<Representation> {
    ...
    const root = identifier.path.split('/')[3]
    const endpointUrl = process.env.SPARQL_STORE_ENDPOINT + "/" + root + "/update"
    const alg = translate(sparql, {quads: true})
    if (alg.insert) {
        alg.insert.forEach((quad: any) => {
            quad["graph"] = {
                termType: 'NamedNode',
    ...
    const endpointQuery = toSparql(alg)
    ...
    this.logger.debug(`${result.size} quads in ${identifier.path}.`);
      ...
      .then(() => this.logger.info('forwarded update to SPARQL store'))
